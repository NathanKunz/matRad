...
/*
Function for transforming the three component arrays of a sparse matrix into 3 sperate vector arrays
compile with matlab: mex -largeArrayDims dissasparse.c
run with matlab: [pr,ir,jc] = dissasparse(sparseArray);
*/
    ...
    int nrhs, const mxArray *prhs[])
{
    ...
    memcpy(new_pr, pr, nnz * sizeof(double));
    mexPrintf("Address of old Array = 0x%x\n\n", pr);
    mexPrintf("Address of new Array = 0x%x\n\n", new_pr);
    //mxSetPr(plhs[0], new_pr);
    ...
    memcpy(new_ir, ir, nnz*sizeof(mwIndex));
    //mxSetPr(plhs[1], mxRealloc(ir, nnz*sizeof(mwIndex)));
    ...
    memcpy(new_jc, jc, (n+1)*sizeof(mwIndex));
    //mxSetPr(plhs[2], mxRealloc(jc, (n+1)*sizeof(mwIndex)));
}
