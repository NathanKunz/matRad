classdef matRad_DoseProjectionMex < matRad_BackProjection
% matRad_DoseProjectionMex class to compute physical dose during optimization
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
    ...
    methods
        function obj = matRad_DoseProjectionMex()
        ...
        function wGrad = projectSingleScenarioGradient(~,dij,doseGrad,scen,~)
            if ~isempty(dij.physicalDose{scen})
                ...
                wGrad = sparseVectorProd(dij.physicalDose{scen}, doseGrad{scen});
            else
                wGrad = [];
                matRad_cfg = MatRad_Config.instance();
                matRad_cfg.dispWarning('Empty scenario in optimization detected! This should not happen...\n');
